describe 'querying the bears table' do
  ...
  it 'selects the most prominent color and returns it with its count' do
    expect(@db.execute("SELECT color, COUNT(*) AS count FROM bears GROUP BY color ORDER BY count DESC LIMIT 1")).to eq([["dark brown", 3]])
  end
  ...
  it 'selects the bear that killed Tim' do
    expect(@db.execute("SELECT * FROM bears WHERE age = 20 AND color = 'black'")).to eq([[8, nil, 20, "M", "black", "aggressive", "FALSE"]])
  end
end
