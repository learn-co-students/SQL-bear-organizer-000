describe 'querying the bears table' do
  ...
  it 'selects all of the bears names and ages that are alive and order them from youngest to oldest' do
    expect(@db.execute('select name, age from bears where alive="TRUE" order by age asc')).to eq([["Grinch", 2], ["Tabitha", 6], ["Wendy", 6], ["Rowdy", 10], ["Melissa", 13]])
  end
  ...
  it 'selects the oldest bear and returns their name and age' do
    ...
    select name, max(age) as max_age
        from bears
      ")).to eq([["Mr. Chocolate", 20]])
  end

  it 'selects the youngest bear and returns their name and age' do
    expect(@db.execute("    select name, min(age) as min_age
        from bears")).to eq([["Grinch", 2]])
  end

  it 'selects the most prominent color and returns it with its count' do
    expect(@db.execute("select color, max(count) from (select color, count(*) as count
      from bears
      group by color)")).to eq([["dark brown", 3]])
  end
